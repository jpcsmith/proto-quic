...
public:
  ...
  virtual void AddSubflow(QuicSubflowId subflowId) = 0;

  enum SendReason {
    ENCRYPTED_TRANSMISSION, UNENCRYPTED_TRANSMISSION
  };

  virtual QuicSubflowId GetNextStreamFrameSubflow(QuicStreamId streamId, size_t length,
      QuicSubflowId hint, SendReason reason) = 0;
  virtual QuicSubflowId GetNextControlFrameSubflow(const QuicFrame& frame,
      QuicSubflowId hint) = 0;

  virtual std::list<QuicSubflowId> AppendAckFrames(
      QuicSubflowId packetSubflowId) = 0;
  virtual void AckFramesAppended(
      const std::list<QuicSubflowId>& ackFrameSubflowIds) = 0;

  virtual void OnAckFrameUpdated(const QuicAckFrame& frame) = 0;
