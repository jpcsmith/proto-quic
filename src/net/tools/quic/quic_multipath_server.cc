...
QuicMultipathServer::QuicMultipathServer(std::unique_ptr<ProofSource> proof_source,
                       QuicHttpResponseCache* response_cache)
    : QuicMultipathServer(std::move(proof_source),
                 QuicConfig(),
...
void QuicMultipathServer::Initialize() {
...
bool QuicMultipathServer::CreateUDPSocketAndListen(const QuicSocketAddress& address) {
...
QuicDefaultPacketWriter* QuicMultipathServer::CreateWriter(int fd) {
  return new QuicDefaultPacketWriter(fd);
}

QuicDispatcher* QuicMultipathServer::CreateQuicDispatcher() {
...
void QuicMultipathServer::Shutdown() {
  if (!silent_close_) {
...
void QuicMultipathServer::OnEvent(int fd, EpollEvent* event) {
  DCHECK_EQ(fd, fd_);
  ...
  if (event->in_events & EPOLLIN) {
    ...
    if(fd_to_writer_map_.find(fd) == fd_to_writer_map_.end()) {
      fd_to_writer_map_[fd] = CreateWriter(fd);
    }
    dispatcher_->SetCurrentWriter(fd_to_writer_map_[fd]);
